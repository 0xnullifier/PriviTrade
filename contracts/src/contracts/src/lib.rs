...
mod tests {
    ...
    fn test_verify_cubic() {
        ...
        let mut proof_file = File::open("src/test_data/cubic/cubic_compressed.proof")
            .unwrap_or_else(|e| {
                panic!("open file error: {}", e);
            });
    ...
    fn test_verify_hasher() {
        ...
        let mut proof_file = File::open("src/test_data/hasher/hasher_compressed.proof")
            .unwrap_or_else(|e| {
                panic!("open file error: {}", e);
            });
        let mut proof_bytes = vec![];
    ...
    fn test_verify_mimc() {
        ...
        let mut proof_file =
            File::open("src/test_data/mimc/mimc_compressed.proof").unwrap_or_else(|e| {
                panic!("open file error: {}", e);
            });
        let mut proof_bytes = vec![];
        proof_file.read_to_end(&mut proof_bytes).unwrap();
    ...
    fn test_compare_cubic_compressed_uncompressed_gnark_proof() {
        let mut compressed_file =
            File::open("src/test_data/hasher/hasher_compressed.proof").unwrap();
        ...
        let mut uncompressed_file =
            File::open("src/test_data/hasher/hasher_uncompressed.proof").unwrap();
        let mut uncompressed_data = Vec::new();
        uncompressed_file
            .read_to_end(&mut uncompressed_data)
            .unwrap();
        let uncompressed_proof =
            proof::Proof::from_uncompressed_gnark_bytes(&uncompressed_data).unwrap();
